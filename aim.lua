    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if not skip then
                ...
                if head and humanoid and humanoid.Health > 0 and root and Camera then
                    -- WorldToViewportPoint 返回 (Vector3, onScreen)
                    local worldPoint, onScreen = Camera:WorldToViewportPoint(head.Position)
                    -- onScreen already embeds z>0 check; but also ensure z>0
                    if onScreen and worldPoint.Z > 0 then
                        local screenPos = Vector2.new(worldPoint.X, worldPoint.Y)
                        if screenPos.X >= 0 and screenPos.Y >= 0 and screenPos.X <= viewportSize.X and screenPos.Y <= viewportSize.Y then
                            -- 计算屏幕距离（中心优先）
...
local function isLikelyProjectile(inst)
    ...
    -- 支持 Model 型投掷物（例如整个模型代表手雷），优先检测 PrimaryPart
    if inst:IsA("Model") then
        local primary = inst.PrimaryPart
        if primary then
            -- 若模型名或 primary 名含关键字，或有速度则视为投掷物
            if nameContainsKeyword(inst.Name) or nameContainsKeyword(primary.Name) then
                return true
            end
            local success, vel = pcall(function() return primary.AssemblyLinearVelocity end)
            if success and typeof(vel) == "Vector3" and vel.Magnitude > 0 then
                return true
            end
            if (primary.Velocity or Vector3.new()).Magnitude > 0 then
                return true
            end
        end
        return false
...
local trackedProjectiles = {} -- [part] = { conn = ..., timeoutCheck = tick() }

local function trackProjectile(part)
    if not part then return end

    -- 如果给的是 Model，则改为追踪 PrimaryPart
    ...
    local meta = { lastUpdate = tick(), timeoutCheck = tick(), conn = nil }
    trackedProjectiles[part] = meta

    local function cleanup()
        if meta.conn then
            pcall(function() meta.conn:Disconnect() end)
            meta.conn = nil
        end
        trackedProjectiles[part] = nil
    end

    meta.conn = part.AncestryChanged:Connect(function(_, parent)
...
heartbeatConn = RunService.Heartbeat:Connect(function(dt)
    ...
    -- 更新连线：从屏幕中间到目标屏幕位置
    if line and Camera then
        local worldPoint, onScreen = Camera:WorldToViewportPoint(targetHead.Position)
        if onScreen and worldPoint.Z > 0 then
            local screenPos = Vector2.new(worldPoint.X, worldPoint.Y)
    ...
    for part, meta in pairs(trackedProjectiles) do
        if not part or not part.Parent or not part:IsDescendantOf(game) then
            ...
            if not ok then
                ...
                local direction = (targetHead.Position - partPos)
                ...
                if meta and meta.timeoutCheck and tick() - meta.timeoutCheck > 25 then
                    if meta.conn then pcall(function() meta.conn:Disconnect() end) end
                    trackedProjectiles[part] = nil
                end
            end
        end
...
addedConn = Workspace.DescendantAdded:Connect(function(desc)
    ...
    -- 支持直接传入 Model（例如整个手雷模型）或 BasePart
...
-- 启动时扫描 Workspace 中的已有物体（支持 Model 和 BasePart）
...
Main:Toggle({
    ...
    Callback = function(state)
        ...
        if not state then
            for part,meta in pairs(trackedProjectiles) do
                if meta and meta.conn then pcall(function() meta.conn:Disconnect() end) end
                trackedProjectiles[part] = nil
            end
            if line then line.Visible = false end
        else
            for _, desc in ipairs(Workspace:GetDescendants()) do
                if (desc:IsA("BasePart") or desc:IsA("Model")) and isLikelyProjectile(desc) then
                    trackProjectile(desc)
                end
            end
        end
...
-- 启动时扫描 Workspace 中的已有物体（支持 Model 和 BasePart）
for _, desc 在 ipairs(Workspace:GetDescendants()) do
    if desc:IsA("Model") then
        if isLikelyProjectile(desc) then
            trackProjectile(desc)
        end
...
Main:Toggle({
    ...
    Callback = function(state)
        ...
        if not state then
            ...
            if line 键，然后 line.Visible = false end
        else
            for _, desc in ipairs(Workspace:GetDescendants()) do
                if (desc:IsA("BasePart") or desc:IsA("Model")) and isLikelyProjectile(desc) then
                    trackProjectile(desc)
