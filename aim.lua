...
local lastTarget
local lastUpdate = 0
local targetUpdateInterval = 0.05 -- 每 0.05 秒更新一次

local function getClosestHead()
    if tick() - lastUpdate < targetUpdateInterval and lastTarget then
        return lastTarget
    end
...
        return { ["FindPartOnRay"] = true }
    elseif mode == "FindPartOnRayWithIgnoreList" then
        return { ["FindPartOnRayWithIgnoreList"] = true }
    elseif mode == "FindPartOnRayWithWhitelist" then
        return { ["FindPartOnRayWithWhitelist"] = true }
    else
        return {}
    end
end
