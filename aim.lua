...
local function updateTarget()
    if main.continuousTracking then
        ...
        elseif main.enablenpc then
            currentTargetHead = getClosestNpcHead()
        else
...
old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    if method == "Raycast" and not checkcaller() then
        ...
        if main.enable or main.enablenpc then
            local targetHead = main.continuousTracking and currentTargetHead or (main.enable and getClosestHead() or getClosestNpcHead())
            
            if targetHead then
                return {
                    Instance = targetHead,
                    Position = targetHead.Position,
                    Normal = (origin - targetHead.Position).Unit,
                    Material = Enum.Material.Plastic,
                    Distance = (targetHead.Position - origin).Magnitude
                }
            end
        end
    end
